...
public class CASignedMitmManager implements MitmManager {
			...
			try {
				...
				if (sslEngineSource == null) {
					sslEngineSource = new CASignedSslEngineSource(domain,
							caPrivateKey, caCertChain, peerCerts);
	...
	private void newRootKeyStore() throws Exception {
		...
		certBldr.addExtension(Extension.keyUsage, true, new KeyUsage(
				KeyUsage.keyCertSign));
		certBldr.addExtension(Extension.subjectKeyIdentifier, false,
				new BcX509ExtensionUtils()
						.createSubjectKeyIdentifier(new SubjectPublicKeyInfo(
								ASN1Sequence.getInstance(keyPair.getPublic()
										.getEncoded()))));
